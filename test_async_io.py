...
async def decode_shape(t):
    return decode_jpeg(torch.frombuffer(t, dtype=torch.uint8)).shape

async def load_batch_async(my_files):
    fs = fsspec.filesystem("abfs", **storage_options)
    # session = await fs.set_session()  # creates client
    fh = fs.cat(my_files)  # fetches data concurrently
    ...
    for k, v in fh.items(): 
        res.append(await decode_shape(v))

    print(res)
    # with f.open() as ff:
    #     print(decode_jpeg(torch.frombuffer(ff.read(), dtype=torch.uint8)).shape)

# load_batch_sync(files[:100])

asyncio.run(load_batch_async(files[:100]))
